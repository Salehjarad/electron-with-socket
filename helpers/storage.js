...
localStorage.prototype.setItem = async (type, item) => {
    try {
        ...
        storage.on('open', () => {
            ...
            const newData =  haveJsonData === '' | haveJsonData === undefined | haveJsonData === null ? false : true;
            if(!newData) {
                return fs.writeFileSync(jsonPath, toJson({token: type === 'token' ? item : '' , id: type === 'id' ? item : ""}))
            }
        })
...
localStorage.prototype.setMulti = async data => {
    try {
      ...
      await fs.writeFileSync(jsonPath, toJson({token: newdata.token, id: newdata.id}))
      return;
    } catch (error) {
        console.log(error)
    }
